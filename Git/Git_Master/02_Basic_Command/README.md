...
작업한 폴더 드래그 드롭한 후 더블클릭

소스토리에서도 History 옵션으로 히스토리 확인 가능

자동적으로 그래프 그려져 있는 것 확인

<br />

방향키로 선택하면 수정된 내용들도 확인 가능
...
깃 히스토리에 커밋들이 점점 많아진다면

`git log` 가 길어지면 특정 부분으로 돌아가기 어렵다

내가 특정한 commit 을 북마크해두고 싶을 때 쓸 수 있는 것이 `git tag`

이렇게 특정한 `commit` 을 `tag` 해놓아서

원하는 부분으로 빠르게 전환 가능

<br />

원하는 문자열을 사용해서 tag 해도 되고

대부분은 제품 릴리즈할 때 그 버전을 tag 많이 해놓는다

<br />

길다란 버전 히스토리에 특정한 부분을 tag 해둬서

언제 릴리즈버전이 되었는지 tag별로 관리가 쉽고 전환이 빠르게 될 수 있도록 도와주는 유용한 북마크 같은
...
또 팀마다 회사마다 다른 버전 시스템을 사용할 수 있겠지만

대부분은 semantic versioning 시스템을 따라 간다

<br />

숫자 3가지를 이용해서 메이저 버전, 마이너 업데이트와 픽스 버전 구분

ex) v2.0.0

<br />

major <- 어떤 특정한 기능이 추가되었을 때, 전체적인 업데이트가 발생했을 때 하나씩 올려가는 버전

minor <- 그 커다란 기능 중에서 조금의 기능들이 업데이트 되거나 개선이 되었을 때 마이너 버전 업데이트

fix <- 기존에 존재하는 기능에서 오류 수정을 했을 때 성능이 조금 개선되었을 때

<br />

어떤 규칙을 사용하느냐가 중요한게 아니라

얼마나 의미있게 일관적으로 사용하는지가 중요
