...
* Traditional Server
  ...
  Thread Pool이 있다(만들 수 있는 쓰레드 제한)

  요청마다 그 요청을 수행할 수 있는 쓰레드 할당

  한 번에 처리할 수 있는 쓰레드 이상이 들어오면 그 요청은 처리할 수 있는 쓰레드 생길 때까지 기다렸다가 처리 다 마치고 Thread Pool 대기하는 쓰레드가 있으면 요청 처리

* Node.js Server

  하나의 메인 싱글 쓰레드가 있고 요청 받은 후 필요한 일들을 처리할 수 있는 네트워크나 데이터베이스에 위임

  ex) 주문 받는 점원(다수의 요청 처리 가능)이 손님 주문 들어오면 티켓을 주고 기다리라고 하고 요리는 주방사가 만들고 음식이 준비되면 서빙하는 사람이 차례대로 티켓에 맞게 요리 전달
